{ pkgs, ... }:
{
  programs.tmux = {
    enable = true;
    package = pkgs.tmux;
    shell = "${pkgs.zsh}/bin/zsh";
    baseIndex = 1;
    terminal = "tmux-256color";
    historyLimit = 10000;
    keyMode = "vi";
    mouse = true;
    escapeTime = 0;

    extraConfig = ''
      # Terminal overrides
      # set -g allow-passthrough on
      set -ga terminal-overrides ",*256col*:Tc"
      # set -ga update-environment TERM
      # set -ga update-environment TERM_PROGRAM
      set -g status-keys vi
      setw -g monitor-activity on
      set -g visual-activity off

      # Set the repeat-time to 1 second so that multiple commands can be entered
      # without sending the prefix-key within 1 second. Commands are made repeatable
      # by -r option of bind-key. By default it is 500 ms.
      set -g repeat-time 500

      set -s focus-events on

      # Reload tmux config
      unbind r
      bind r source-file ~/.config/tmux/tmux.conf \; display "Reloaded tmux config"

      # Don't prompt for killing panes and windows
      bind-key x kill-pane
      bind-key X kill-window

      ###############################################
      ## Copy & Paste setting
      ###############################################
      bind -n M-Space copy-mode
      bind P paste-buffer
      bind-key -T copy-mode-vi v send-keys -X begin-selection
      bind-key -T copy-mode-vi y send-keys -X copy-selection
      bind-key -T copy-mode-vi r send-keys -X rectangle-toggle
      bind -T copy-mode-vi Enter send-keys -X copy-pipe-and-cancel "xclip -i -f -selection primary | xclip -i -selection clipboard"

      ###############################################
      ## Window Settings
      ###############################################

      # A zsh chpwd hook (see zsh.nix) stores the git-project name (or dir
      # basename) in the @auto_window_name option; automatic-rename renders it,
      # falling back to the path basename when the option is unset.
      #
      # Because this goes through automatic-rename, running `tmux rename-window`
      # (or `prefix R`) turns automatic-rename OFF for that window, so a manual
      # name sticks -- including when new panes open in the window. To restore
      # auto-naming: tmux setw -u automatic-rename (then cd).
      set-window-option -g automatic-rename on
      set-option -g automatic-rename-format '#{?@auto_window_name,#{@auto_window_name},#{b:pane_current_path}}'
      set-option -g set-titles on

      bind R command-prompt -I "#W" "rename-window -- '%%'"

      # use vim keys for splitting
      bind-key v split-window -h -c "#{pane_current_path}"
      bind-key V split-window -v -c "#{pane_current_path}"

      # Shift arrow to switch windows
      bind -n S-Left  previous-window
      bind -n S-Right next-window

      ###############################################
      ## Pane Settings
      ###############################################

      # Use shift hjkl to resize
      bind-key -r J resize-pane -D 5
      bind-key -r K resize-pane -U 5
      bind-key -r H resize-pane -L 5
      bind-key -r L resize-pane -R 5

      # Vim style pane selection
      bind h select-pane -L
      bind j select-pane -D
      bind k select-pane -U
      bind l select-pane -R

      vim_pattern='(\S+/)?g?\.?(view|l?n?r?vim?x?|fzf)(diff)?(-wrapped)?'
      is_vim="ps -o state= -o comm= -t '#{pane_tty}' \
          | grep -iqE '^[^TXZ ]+ +'\"$vim_pattern\"'$'"
      bind-key -n 'C-h' if-shell "$is_vim" 'send-keys C-h'  'select-pane -L'
      bind-key -n 'C-j' if-shell "$is_vim" 'send-keys C-j'  'select-pane -D'
      bind-key -n 'C-k' if-shell "$is_vim" 'send-keys C-k'  'select-pane -U'
      bind-key -n 'C-l' if-shell "$is_vim" 'send-keys C-l'  'select-pane -R'
      tmux_version='$(tmux -V | sed -En "s/^tmux ([0-9]+(.[0-9]+)?).*/\1/p")'
      # bind-key -n C-\\ if-shell "$is_vim" 'send-keys C-\\'  'select-pane -l'

      bind-key -T copy-mode-vi 'C-h' select-pane -L
      bind-key -T copy-mode-vi 'C-j' select-pane -D
      bind-key -T copy-mode-vi 'C-k' select-pane -U
      bind-key -T copy-mode-vi 'C-l' select-pane -R
      bind-key -T copy-mode-vi 'C-\' select-pane -R

      # Use Alt-vim keys without prefix key to switch panes
      # bind -n M-h select-pane -L
      # bind -n M-j select-pane -D
      # bind -n M-k select-pane -U
      # bind -n M-l select-pane -R

      # Unbind conflicting plugin keybindings
      # unbind -n M-h
      # unbind -n M-j
      # unbind -n M-k
      # unbind -n M-l
      # unbind -n M-Up
      # unbind -n M-Down
      # unbind -n M-Left
      # unbind -n M-Right
      # unbind -n C-h
      # unbind -n C-j
      # unbind -n C-k
      # unbind -n C-l

      # Quick pane cycling
      bind -r TAB select-pane -t :.+

      unbind ,
      bind , swap-pane -D # swap current pane with next one
      bind . swap-pane -U # swap current pane with previous one

      bind > swap-window -t +1 -d # swap current window with next one
      bind < swap-window -t -1 -d # swap current window with previous one

      ###############################################
      ## Theme Settings
      ###############################################
      set -g status-interval 5
      set -g status on

      set -g status-justify left

      #+--- Colors ---+
      set -g status-style "fg=#c6c8d1,bg=#0f1117"

      set -g pane-border-style "fg=#2e3244,bg=#161821"
      set -g pane-active-border-style "fg=#6b7089,bg=#161821"
      set -g window-style 'fg=#c6c8d1,bg=#161821'
      set -g display-panes-colour white
      set -g display-panes-active-colour brightblack

      #+------------+
      #+ Clock Mode +
      #+------------+
      setw -g clock-mode-colour cyan

      #+----------+
      #+ Messages +
      #+---------+
      set -g message-style "fg=#c6c8d1,bg=#2e3244"
      set -g message-command-style "fg=#c6c8d1,bg=#2e3244"

      #+--------+
      #+ Status +
      #+--------+
      #+--- Bars ---+
      set -g status-left "#[fg=#c6c8d1,bg=black,bold] #S #[fg=black,bg=brightblack,nobold,noitalics,nounderscore]"
      # M8 usage segment rides inside `vigil status`; the default 40-col cap clips it.
      # Date/time dropped (2026-07-14): redundant in tmux, and it crowded the segment.
      set -g status-right-length 150
      set -g status-right "#(~/projects/rust/vigil/.nix-cargo/bin/vigil status)#{prefix_highlight}"

      #+--- Windows ---+
      set -g window-status-format "#[fg=black,bg=#6b7089,nobold,noitalics,nounderscore] #[fg=black,bg=#6b7089]#I #[fg=black,bg=#6b7089,nobold,noitalics,nounderscore] #[fg=black,bg=#6b7089]#W #F #[fg=#6b7089,bg=#0f1117,nobold,noitalics,nounderscore]"
      set -g window-status-current-format "#[fg=black,bg=#c6c8d1,nobold,noitalics,nounderscore] #[fg=black,bg=#c6c8d1]#I #[fg=black,bg=#c6c8d1,nobold,noitalics,nounderscore] #[fg=black,bg=#c6c8d1]#W #F #[fg=#c6c8d1,bg=#0f1117,nobold,noitalics,nounderscore]"
      set -g window-status-separator ""

      # Agent Monitor (vigil)
      bind-key a display-popup -E -w 80% -h 60% "~/projects/rust/vigil/.nix-cargo/bin/vigil ui"
      bind-key -T root MouseDown1StatusRight display-popup -E -w 80% -h 60% "~/projects/rust/vigil/.nix-cargo/bin/vigil ui --usage"

    '';

    plugins = with pkgs.tmuxPlugins; [
      {
        plugin = resurrect;
        extraConfig = ''
          set -g @resurrect-capture-pane-contents 'on'
        '';
      }
      open
      copycat
      yank
      {
        plugin = prefix-highlight;
        extraConfig = ''
          set -g @prefix_highlight_fg 'white'
          set -g @prefix_highlight_bg 'blue'
        '';
      }
      {
        plugin = continuum;
        extraConfig = ''
          set -g @continuum-restore 'on'
          set -g @continuum-save-interval '15'
        '';
      }
    ];
  };
}
