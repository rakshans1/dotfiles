...
function plugin_loader.init(opts)
  ...
  if status_ok then
    packer.on_complete = vim.schedule_wrap(function()
      require("utils.hooks").run_on_packer_complete()
    end)
...
function plugin_loader.load_snapshot(snapshot_file)
  ...
  if not in_headless then
    vim.notify("Syncing core plugins is in progress..", vim.log.levels.INFO, { title = "rvim" })
  end
...
function plugin_loader.sync_core_plugins()
  ...
  vim.cmd [[autocmd User PackerComplete ++once lua require("plugin-loader").load_snapshot() ]]
  pcall_packer_command "sync"
end
